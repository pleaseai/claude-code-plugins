...
// The multi-format generator now lives inside the plugin-dev plugin so that
// plugin is self-contained. Re-export it here so the historical
// `bun scripts/cli.ts multi-format` invocation keeps working.
import { generateMultiFormat } from "../plugins/plugin-dev/scripts/run.ts"

export { generateMultiFormat }
...
// multi-format
// ---------------------------------------------------------------------------
// The generator now lives in plugins/plugin-dev/scripts/ (run.ts + multi-format.ts)
// so the plugin is self-contained. `generateMultiFormat` is re-exported from the
// import block at the top of this file; the `multi-format` command below delegates
// to it.
