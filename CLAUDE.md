...
**2a. Multi-runtime manifests (Codex + Antigravity + Cursor)**

Each local plugin in `plugins/<name>/` ships runtime manifests so the same
directory installs in Claude Code, Codex, Google Antigravity, and Cursor:
...
| Cursor       | `.cursor-plugin/plugin.json`           | none — components auto-discovered, mcpServers inline |

The Claude manifest is the source of truth — author plugins there. Codex,
Antigravity, and Cursor artifacts are generated by `bun scripts/cli.ts multi-format`
and should not be hand-edited. See `scripts/multi-format.ts` for the conversion rules.

After modifying a plugin's Claude manifest or adding a new local plugin, run:

```bash
bun scripts/cli.ts multi-format
```

Codex marketplace is auto-generated at `.agents/plugins/marketplace.json` and the
Cursor marketplace at `.cursor-plugin/marketplace.json`, both from the Claude
marketplace; only the local plugins (`source: "./plugins/..."`) are included.
